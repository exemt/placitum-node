...
// EnvelopeVersion -- первый байт конверта. Версия отделяет схему обёртки DEK:
// разбор RSA-OAEP не должен молча съесть будущий HPKE.
const EnvelopeVersion = 0x01

// headerLen -- версия и длина обёрнутого DEK.
const headerLen = 3
...
// DecryptEnvelope opens a hybrid-encrypted blob:
// [version (1)][wrapped DEK length, big-endian uint16][wrapped DEK][nonce (12)][ciphertext+tag]
// DEK is unwrapped with RSA-OAEP SHA-256, payload decrypted with AES-256-GCM.
//
// Длина обёрнутого DEK едет в заголовке явно, чтобы ротация ключа контура на
// другой размер не меняла разбор: RSA-4096 даёт постоянные 512 байт сейчас,
// но формат на это число не завязан. См. docs/config-distribution.md.
