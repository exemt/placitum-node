/*
 * Настройки агента с контроллера: KV policy/agent-conf.
 *
 * Документ мелкий и не секретный — адрес архива, регион, бакеты и темп
 * выгрузки, — поэтому он лежит в значении ключа целиком: ни блобов в Redis,
 * ни расшифровки, в отличие от поколения nginx.
 *
 * Что сюда не приезжает и приезжать не может:
 *
 *   * адрес шины, контроллер, ключ ноды, сокет вердиктов — ими агент
 *     дотягивается до самой шины, и доставить их шиной нельзя по кругу
 *     зависимостей;
 *   * реквизиты S3 — ключи остаются секретом ноды
 *     (WAF_RETAIN_S3_CREDENTIALS_FILE). Контроллер называет хранилище, но не
 *     открывает его.
 *
 * Поэтому документ — оверлей поверх файла и окружения, а не замена им.
 */
...
/*
 * Overlay кладёт документ поверх базовой настройки ноды.
 *
 * База — то, что дали файл и окружение: адрес обменника, путь к реквизитам, уже
 * прочитанные ключи. Документ перекрывает только названное в нём: пустой
 * endpoint в документе означает «контроллер про это ничего не говорит»,
 * а не «выключить архив». Выключают архив, стирая endpoint у ноды, — иначе
 * первое же поколение без секции s3 гасило бы архив на всём флоте.
 */
func (c *AgentConf) Overlay(base retain.Config) retain.Config {
	out := base

	// Карты у базы общие с прежней конфигурацией: копия обязательна, иначе
	// оверлей правит настройку работающего пула.
