...
type NginxPack struct {
	...
	// Pages -- страницы отказа: имя файла -> хеш тела. Как files у правил,
	// а не как store: содержимое не секрет, шифровать его незачем, и ключ
	// контура для страницы отказа значил бы, что её нельзя посмотреть в
	// каталоге контроллера.
...
// safePageName держит имя в пределах одного файла каталога страниц: манифест
// приходит с шины, и "../nginx.conf" в нём не должен становиться путём.
