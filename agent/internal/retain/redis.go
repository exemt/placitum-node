/*
 * Клиент обменника: GET и DEL, больше ничего.
 *
 * Свой, а не готовый, по той же причине, по которой свой у модуля
 * (nginx/module/src/body/drv_redis.c): проводной протокол RESP крошечный, а
 * зависимость тянет за собой пулы, метрики и режимы кластера, которых здесь
 * не нужно ни одного. Разница только в том, что агенту можно блокировать: он
 * не в цикле событий nginx, и его воркер ничего, кроме себя, не держит.
 *
 * Соединение живёт в воркере и переоткрывается при любой ошибке. Разделяемого
 * пула нет намеренно: воркеров единицы, а без общего состояния нет и замка.
 */
...
// ErrMissing — ключа в обменнике нет. Не отказ: горячий слой живёт минуты, и
// объект мог просто не дождаться.
...
// redisPool — соединения одного воркера, по одному на адрес узла. Адрес берётся
// из подсказки локатора: узел, на который лёг ключ, знает только тот, кто его
// клал.
...
// get возвращает ErrMissing, если ключа нет: пустое значение и отсутствие
// ключа — разные события, и склеивать их значило бы объявлять пустое тело
// потерянным.
...
// call пишет команду и читает один ответ. Ошибка закрывает соединение: разбор
// RESP позиционный, и продолжать на конвейере, чьё состояние неизвестно,
// значит однажды прочитать чужой ответ.
...
// readReply — один ответ RESP. Нужны четыре формы из пяти: массивов среди
// ответов на GET и DEL не бывает.
func readReply(br *bufio.Reader) (byte, []byte, error) {
	...
	case '$':
		...
		// +2 — завершающий CRLF, он часть кадра, а не значения.
		data := make([]byte, n+2)
