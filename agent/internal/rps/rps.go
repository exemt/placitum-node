/*
 * Скользящее окно по итогам с unix-сокета. Десять секундных корзин:
 * сумма за окно / 10. Всего и по классу ответа 2xx / 3xx / 4xx / 5xx.
 * Пульс читает Rate и Status, воркеры только Add.
 *
 * На allow фазы запроса модуль шлёт status=0 — апстрим ещё не ответил.
 * Тогда класс берём из вердикта: allow→2xx, redirect→3xx, deny→4xx.
 */
...
// Class — 2..5 по коду ответа. 0, если класс неизвестен.
