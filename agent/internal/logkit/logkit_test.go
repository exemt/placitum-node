...
// Словарь один на весь контур: слово, которое понял инспектор, обязан понять
// и сервис, иначе одна строка документа значит разное у соседей.
func TestParseDictionary(t *testing.T) {
	...
	for word, want := range cases {
		got, err := Parse(word)
		if err != nil || got != want {
			t.Fatalf("Parse(%q) = %v, %v; ждали %v", word, got, err, want)
		}
	}

	if _, err := Parse("emerg"); err == nil {
		t.Fatal("emerg принят: в словаре процесса его нет")
	}

	if String(Notice) != "notice" || String(slog.LevelDebug) != "debug" {
		t.Fatal("String не словами nginx")
	}
}
