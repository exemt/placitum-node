/*
 * Селекторы запроса и условия if.
 *
 * Селектор ($waf_request_cookies.sid) намеренно не является переменной nginx:
 * имя переменной набирается только из A-Za-z0-9_, и в log_format такая запись
 * распалась бы на $waf_request_cookies и текст ".sid". Разбирают его директивы
 * модуля, поэтому имя пары может быть любым -- с дефисом, точкой, в своём
 * регистре, -- то есть таким, каким оно пришло с провода.
 *
 * Селектор даёт множество значений, а не первое. ?sid=ok&sid=inject -- два
 * значения, и набор спрашивается о каждом: проверка, смотрящая только на первое
 * вхождение, обходится добавлением одного параметра.
 *
 * Разбор строки запроса и Cookie кэшируется в контексте: условия одного запроса
 * обязаны видеть одно и то же, а волна из пяти инспекторов иначе перебирала бы
 * аргументы пять раз.
 */
    ...
    ngx_http_waf_operand_t *op, ngx_uint_t all, const char *directive)
{
    ...
    /*
     * Чужая фаза ловится отдельным сообщением: $waf_response_headers.x -- это не
     * опечатка, а запись про фазу, которой ещё нет, и молчаливое "неизвестный
     * объект" отправило бы искать ошибку не туда.
     */
    ...
    /* Имя берётся целиком после первой точки: кука sess.id -- одно имя. */
    name.data = dot + 1;
    ...
    if (name.len == 1 && name.data[0] == '*') {
        ...
        /*
         * Ключ счётчика частоты обязан быть одним значением: множество означало
         * бы, что один запрос считается в несколько счётчиков сразу, и лимит
         * перестал бы значить то, что написано в rate=.
         */
        if (!all) {
            ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
    ...
    const char *directive)
{
    ...
    /* if <значение> in <набор>  |  if <значение> not in <набор> */
    ...
    /*
     * Набор обязан быть объявлен выше по файлу -- как и у waf_local_check.
     * Опечатка в имени иначе становится условием, которое никогда не сходится,
     * то есть тихо снятой проверкой.
     */
...
/*
 * Строка запроса. Имя и значение декодируются: сравнение со списком идёт с тем,
 * что увидит приложение, а не с тем, как это записано на проводе, -- иначе
 * %27 обходит любой список.
 */
static ngx_int_t
...
/*
 * Cookie. Декода нет: значение куки -- непрозрачная строка приложения, и
 * раскрывать в ней проценты значило бы сравнивать со списком не то, что
 * приложение прочтёт. Пробелы и кавычки по краям снимаются: они часть
 * синтаксиса заголовка, а не значения.
 */
static ngx_int_t
...
/* Один заголовок Cookie: пары через ';', пробелы и кавычки по краям снимаются. */
static ngx_int_t
...
/*
 * Полное раскрытие процентов, включая управляющие байты: список сравнивается с
 * тем, что получится у приложения. "+" -- пробел: строка запроса читается по
 * правилам формы, а не URI.
 */
static ngx_int_t
...
ngx_http_waf_pair_named(ngx_http_waf_operand_t *op, ngx_http_waf_pair_t *pair)
{
    ...
    /*
     * Имя заголовка регистронезависимо по HTTP; имена аргумента и куки -- нет:
     * приложение различает sid и SID, и список обязан различать так же.
     */
    ...
    ngx_http_waf_operand_t *op, ngx_str_t *out)
{
    ...
    return NGX_OK;                     /* пары нет: пустой ключ */
}
...
ngx_http_waf_cond_test(ngx_http_waf_ctx_t *ctx, ngx_array_t *conds)
{
    ...
    /* Несколько условий -- И: строка работает, только если сошлись все. */
    for (i = 0; i < conds->nelts; i++) {
        ...
        /*
         * Пустое значение -- это "не в наборе", а не отдельный случай: куки нет
         * -- значит, её нет и в списке доверенных. Иначе "not in" на
         * отсутствующей куке был бы ложным, и клиент без куки -- самый
         * подозрительный -- оставался бы без проверки, которую условие как раз
         * и включает.
         */
        if (cond[i].negate) {
            if (rc == NGX_OK) {
