/*
 * Соединение с внешним сервисом: см. ngx_http_waf_link.h.
 */
    ...
    ngx_http_waf_link_conf_t *conf, ngx_cycle_t *cycle, void *data)
{
     ...
     * неотменяемых таймеров, а переподключение перевзводится вечно. Без флага
     * воркер не выходит никогда -- ни reload, ни остановка не завершаются.
     */
...
ngx_http_waf_link_connect(ngx_http_waf_link_t *link)
{
    ...
    if (rc == NGX_OK) {
        /* соединение установилось сразу: локальный сокет */
...
ngx_http_waf_link_close(ngx_http_waf_link_t *link)
{
    ...
    /*
     * Готовность снимается до колбэка: владелец, разбирая конвейер, может
     * завершать чужие операции, а те -- искать соединение под новую команду.
     * В закрывающееся соединение они попасть не должны.
     */
...
ngx_http_waf_link_reschedule(ngx_http_waf_link_t *link)
{
    ...
    /*
     * Переподключение вечное и без экспоненты: и шина, и обменник -- обязательные
     * части контура, а не необязательные апстримы; их недоступность
     * обрабатывается политикой на каждом запросе, а не отказом клиента.
     */
...
ngx_http_waf_link_read_handler(ngx_event_t *rev)
{
    ...
    for ( ;; ) {
        ...
        if (link->in_last == link->in_size) {
            ...
            /* сдвиг остатка к началу: разобранное место освободилось */
...
ngx_http_waf_link_reserve(ngx_http_waf_link_t *link, size_t len)
{
    ...
    /* сдвиг неотправленного остатка к началу буфера */
    if (link->out_pos != 0) {
    ...
    /*
     * Предел роста задаёт владелец: у шины это pending_max, у обменника -- одно
     * тело максимального размера с запасом на команду. Больше означало бы,
     * что воркер копит в памяти вместо того, чтобы честно отказать.
     */
