/*
 * Писатель и парсер JSON без аллокаций в горячем пути.
 *
 * Сторонняя библиотека здесь была бы худшим решением из возможных: это
 * единственное место модуля, куда приходят байты от внешнего сервиса, и
 * поверхность атаки должна быть обозримой целиком. Поддерживается ровно то
 * подмножество, которое встречается в протоколе вердикта.
 */
...
/*
 * Кодовая точка в UTF-8. В nginx такой функции нет: ngx_utf8_decode работает в
 * обратную сторону, а раскрытие \u нам нужно именно вперёд.
 */
...
/*
 * Строка в кавычках. Экранируются кавычка, обратный слеш и всё, что меньше
 * 0x20: непроэкранированный управляющий символ ломает разбор у получателя, а
 * получателей у нас столько, сколько инспекторов.
 *
 * Байты старше 0x7f отдаются как есть: JSON допускает UTF-8, а проверять
 * корректность кодировки заголовков запроса не наша задача -- инспектор видит
 * их такими, какими они приехали.
 */
void
...
/*
 * Разделитель перед следующим членом. Запятая необязательна перед первым, но
 * обязательна перед остальными; отслеживать, первый ли это, не нужно: после
 * запятой обязано идти значение, а не закрывающая скобка, и это проверяется
 * вызывающим кодом естественным образом.
 */
static ngx_int_t
...
ngx_http_waf_jp_string(ngx_http_waf_jp_t *jp, ngx_str_t *out)
{
    ...
    /*
     * Раскрытие escape-последовательностей только сокращает строку, поэтому
     * остатка буфера гарантированно достаточно и второго прохода не нужно.
     */
...
/*
 * Пропуск строки без раскрытия: нужен там, где строка не влезла в буфер, а
 * разбор обязан продолжиться со следующего элемента. Экранированный символ
 * проскакивается парой: после обратной косой не бывает ни закрывающей кавычки,
 * ни начала новой последовательности, а hex-цифры -- обычные байты.
 */
static ngx_int_t
...
/*
 * Строка в буфер вызывающего. Нужна там, где строк много и заранее известна их
 * верхняя длина: копия в пул на каждый элемент массива из миллиона записей --
 * это миллион аллокаций размером с остаток сообщения.
 *
 * Не влезла -- NGX_DECLINED и пустой out, а разбор стоит на следующем
 * элементе. Ошибкой это было раньше, и одна длинная запись роняла весь кадр:
 * набор оставался пустым вместе со всеми проверками по нему. Что делать с
 * пропущенной записью, решает вызывающий -- он один знает, чей это набор.
 */
ngx_int_t
    ...
    ngx_str_t *out)
{
    ...
    while (jp->pos < jp->end) {
        ...
        /*
         * Дальше идёт раскрытие escape-последовательности, а кодовая точка вне
         * BMP занимает четыре байта: запас проверяется по худшему случаю, чтобы
         * не проверять его в каждой ветви ниже.
         */
        if ((size_t) (last - p) < 4) {
            ngx_str_null(out);
        ...
        /*
         * Суррогатная пара. Одиночный суррогат не отвергается, а кодируется как
         * есть: смысл раскрытия \u здесь -- не пропустить управляющий символ
         * или CR/LF в обход проверки значений, а не выдать безупречный UTF-8.
         */
            ...
            && jp->pos[0] == '\\' && jp->pos[1] == 'u')
        {
            ...
            if (low >= 0xdc00 && low <= 0xdfff) {
                ...
                /* не пара: второй символ кодируем отдельно ниже */
...
ngx_http_waf_jp_int(ngx_http_waf_jp_t *jp, ngx_int_t *out)
{
    ...
    /*
     * Дробное и экспоненциальная запись отвергаются, а не округляются: поле
     * score с точкой -- это ошибка инспектора, и молча превращать её в решение
     * нельзя.
     */
    if (jp->pos < jp->end
...
ngx_http_waf_jp_skip(ngx_http_waf_jp_t *jp)
{
    ...
    for ( ;; ) {
        ...
        default:
            /*
             * Число пропускаем посимвольно, а не через jp_int: дробное значение
             * в незнакомом поле -- не наша забота, отвергать надо только там,
             * где мы это значение используем.
             */
            if (c != '-' && (c < '0' || c > '9')) {
